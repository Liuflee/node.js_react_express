...
const App = () => {
  ...
  const callAPI = (search) => {
    if (search.trim() !== '') { // Verificar que el término de búsqueda no esté vacío
  ...
  return (
    <div className="app-container"> {/* Aplica estilos al contenedor principal */}
      <h1 className="app-title">Aplicación de Búsqueda de Noticias</h1>
      <SearchBar onSearch={callAPI} />
      <div className="news-container"> {/* Aplica estilos al contenedor de noticias */}
        <h2 className="news-title">Noticias</h2>
        <ul className="news-list">
          {newsData.map((article, index) => (
            <li key={index} className="news-item"> {/* Aplica estilos a cada noticia */}
